	  ...
	  var methods = {
		init: function(options) {
			return this.each(function() {
				...
				var defaluts = {
						...
						hoverColor:'#cccccc',//鼠标放上去时的背景色
						borderColor:'#bebebe',//边框线颜色
						isShowPage:true, //是否显示页码器
				...
				var METHOD = {
					...
					getTableAutoHeight:function(parentDiv,suffix){
						...
						if(METHOD.inH($("#dataTable",parentDiv))>maxTableHeight){ //判断是否显示y轴滚动条，
							...
							$("#dataDiv"+suffix,parentDiv).css("width",(winWidth)+'px');							
							$("#dataDiv"+suffix,parentDiv).css({"height":maxTableHeight+'px',"max-height":maxTableHeight+'px'});
							//$("#dataDiv"+suffix,parentDiv).css({"max-height":"","height":""});
		...
		tableRender:function(){
			return this.each(function(){
				...
				if($("#"+dataTableId).height()>$("#"+dataDivId).height()) flowY = true;
				...
				if(setting.isCovered){
				    ...
				    if(totalw>width){
				  	    $("#"+headDivId).css('width',width-12+"px");
					    $("#"+headTableId).css('width',totalw-12+"px");
					    $("#"+dataTableId).css('width',totalw+"px");
				  	}else{
					 	$("#minWidth"+setting.suffix).val(width-12);
					  	if(flowY){ //当有纵向滚动条时
					  		$("#"+headTableId).css('width',width-12+"px");         
					     	$("#"+dataTableId).css('width',width-12+"px");
					  	}else{
					...
					var width = parseInt($("#"+dataDivId).parent().width());
					    totalw = parseInt($("#"+dataTableId).width());
					$("#minWidth"+setting.suffix).val(totalw-12);
					if(flowY){
